...
#ifdef DEBUG
#define DBGSTR(message)     output->print(message)
#define DBGBUF(buf, len)	output->write(buf, len)
#define DBGLN(message)		output->println(message)
#define DBGFMT(msg, fmt)	output->print(msg, fmt)
#define DBGCHAR(c)			output->write(c)
#else
#define DBG(message)
#endif // DEBUG
...
class VR {
public:
	VR(uint8_t receivePin, uint8_t transmitPin, Stream* serialBt);
