...
*/
VR::VR(uint8_t receivePin, uint8_t transmitPin, Stream* serialBt) {
    instance = this;
    output = serialBt;
}
...
int VR::clear() {
    ...
    if (len <= 0) {
        output->println("Clear returned nothing");
        return -1;
    }

    if (vr_buf[2] != FRAME_CMD_CLEAR) {
        output->println("Clear returned ");
        output->print(vr_buf[0], HEX);
        output->print(vr_buf[1], HEX);
        output->print(vr_buf[2], HEX);
        output->print(vr_buf[3], HEX);
        output->print("\n");
        return -1;
    }
