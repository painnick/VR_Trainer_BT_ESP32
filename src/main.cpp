 ...
 * TXD2(17)   ------->     RX
 */
VR myVR(16, 17, &SerialBT);    // 2:RX 3:TX, you can choose your favourite pins.
...
void setup(void) {
    ...
    SerialBT.begin("VR-Trainer");
    SerialBT.register_callback([](esp_spp_cb_event_t evt, esp_spp_cb_param_t *param) {
        switch (evt) {
            case ESP_SPP_SRV_OPEN_EVT:
                SerialBT.println(F("Elechouse Voice Recognition V3 Module \"train\" sample."));

                printSeperator();
                SerialBT.println(F("Usage:"));
                printSeperator();
                printHelp();
                printSeperator();
                break;
...
void loop(void) {
    ...
    if (len > 0) {
        ...
        if (!checkCMD(len)) {
                ...
                if (paraLen == cmdLen[i]) {
                    ...
                    if (compareCMD(paraAddr, (uint8_t *) cmdList[i], paraLen) == 0) {
                        ...
                        if (cmdFunction[i](len, paraNum) != 0) {
                            printSeperator();
                            SerialBT.println(F("Command Format Error!"));
                            printSeperator();
                        }
                        break;
                    }
            ...
            if (i == CMD_NUM) {
                printSeperator();
                SerialBT.println(F("Unkonwn command"));
                printSeperator();
            }
        } else {
...
int receiveCMD() {
    ...
    if (!recv.isEmpty()) {
        ESP_LOGI(MAIN_TAG, "RECV >> %s", recv.c_str());
        len = recv.length();
        strcpy(reinterpret_cast<char *>(cmd), recv.c_str());
        cmd[len] = '\n';
        len++;
    } else {
        return -1;
...
int cmdTrain(int len, int paraNum) {
    ...
    if (ret >= 0) {
        ...
        SerialBT.println(F("Train failed."));
    } else if (ret == -2) {
        SerialBT.println(F("Train Timeout."));
    }
    printSeperator();
    return 0;
}
...
int cmdLoad(int len, int paraNum) {
    ...
    if (ret >= 0) {
        ...
        SerialBT.println(F("Load failed or timeout."));
    }
    printSeperator();
    return 0;
}
...
int cmdClear(int len, int paraNum) {
    ...
    if (myVR.clear() == 0) {
        printSeperator();
        SerialBT.println(F("Recognizer cleared."));
        printSeperator();
    } else {
        printSeperator();
        SerialBT.println(F("Clear recognizer failed or timeout."));
        printSeperator();
    }
    return 0;
}
...
int cmdVR(int len, int paraNum) {
    ...
    if (ret <= 0) {
        printSeperator();
        SerialBT.println(F("Check recognizer failed or timeout."));
        printSeperator();
        return 0;
    }
    printSeperator();
...
int cmdRecord(int len, int paraNum) {
    ...
    if (paraNum == 1) {
        ...
        if (ret >= 0) {
            ...
            SerialBT.println(F("Check record failed or timeout."));
        }
        printSeperator();
    } else if (paraNum < 9) {
...
int cmdSigTrain(int len, int paraNum) {
    ...
    if (ret >= 0) {
        ...
        SerialBT.println(F("Train with signature failed or timeout."));
    }
    printSeperator();
...
int cmdGetSig(int len, int paraNum) {
    ...
    printSeperator();
    if (ret == 0) {
        SerialBT.println(F("Signature isn't set."));
    } else if (ret > 0) {
        SerialBT.print(F("Signature:"));
        printSignature(buf, ret);
        SerialBT.println();
    } else {
        SerialBT.println(F("Get sig error or timeout."));
    }
    printSeperator();
...
int cmdTest(int len, int paraNum) {
    printSeperator();
    SerialBT.println(F("TEST is not supported."));
    printSeperator();
    return 0;
}
...
int cmdSettings(int len, int paraNum) {
    ...
    if (ret > 0) {
        ...
        printSeperator();
        SerialBT.println(F("Check system settings error or timeout"));
        printSeperator();
    }
    return 0;
}
 ...
 */
void printVR(uint8_t *buf) {
    SerialBT.println(F("VR Index\tGroup\tRecordNum\tSignature"));

    SerialBT.print(buf[2], DEC);
    SerialBT.print(F("\t\t"));

    if (buf[0] == 0xFF) {
        SerialBT.print(F("NONE"));
    } else if (buf[0] & 0x80) {
        SerialBT.print(F("UG "));
        SerialBT.print(buf[0] & (~0x80), DEC);
    } else {
        SerialBT.print(F("SG "));
        SerialBT.print(buf[0], DEC);
    }
    SerialBT.print(F("\t"));

    SerialBT.print(buf[1], DEC);
    SerialBT.print(F("\t\t"));
    if (buf[3] > 0) {
 ...
 */
void printCheckRecognizer(uint8_t *buf) {
    SerialBT.print(F("All voice records in recognizer: "));
    SerialBT.println(buf[8], DEC);
    SerialBT.print(F("Valid voice records in recognizer: "));
    SerialBT.println(buf[0], DEC);
    if (buf[10] == 0xFF) {
        SerialBT.println(F("VR is not in group mode."));
    } else if (buf[10] & 0x80) {
        SerialBT.print(F("VR is in user group mode:"));
        SerialBT.println(buf[10] & 0x7F, DEC);
    } else {
        SerialBT.print(F("VR is in system group mode:"));
        SerialBT.println(buf[10], DEC);
    }
    SerialBT.println(F("VR Index\tRecord\t\tComment"));
    for (int i = 0; i < 7; i++) {
        SerialBT.print(i, DEC);
        SerialBT.print(F("\t\t"));
        if (buf[i + 1] == 0xFF) {
            if (buf[10] == 0xFF) {
                SerialBT.print(F("Unloaded\tNONE"));
            } else {
                SerialBT.print(F("Not Set\t\tNONE"));
            }
        } else {
            SerialBT.print(buf[i + 1], DEC);
            SerialBT.print(F("\t\t"));
            if (buf[9] & (1 << i)) {
                SerialBT.print(F("Valid"));
            } else {
                SerialBT.print(F("Untrained"));
            }
        }
 ...
 */
void printCheckRecord(uint8_t *buf, int num) {
    SerialBT.print(F("Check "));
    SerialBT.print(buf[0], DEC);
    SerialBT.println(F(" records."));

    SerialBT.print(num, DEC);
    if (num > 1) {
        SerialBT.println(F(" records trained."));
    } else {
        SerialBT.println(F(" record trained."));
    }
    ...
    for (int i = 0; i < buf[0] * 2; i += 2) {
        ...
        switch (buf[i + 2]) {
            case 0x01:
                SerialBT.print(F("Trained"));
                break;
            case 0x00:
                SerialBT.print(F("Untrained"));
                break;
            case 0xFF:
                SerialBT.print(F("Record value out of range"));
                break;
            default:
                SerialBT.print(F("Unknown Stauts"));
                break;
        }
        SerialBT.println();
    }
...
void printUserGroup(uint8_t *buf, int len) {
    int i, j;
    SerialBT.println(F("Check User Group:"));
    for (i = 0; i < len; i++) {
        SerialBT.print(F("Group:"));
        SerialBT.println(buf[8 * i]);
        for (j = 0; j < 7; j++) {
            if (buf[8 * i + 1 + j] == 0xFF) {
                SerialBT.print(F("NONE\t"));
            } else {
                SerialBT.print(buf[8 * i + 1 + j], DEC);
                SerialBT.print(F("\t"));
            }
        }
        SerialBT.println();
...
void printLoad(uint8_t *buf, uint8_t len) {
    if (len == 0) {
        SerialBT.println(F("Load Successfully."));
        return;
    } else {
        SerialBT.print(F("Load success: "));
        SerialBT.println(buf[0], DEC);
    }
    for (int i = 0; i < len - 1; i += 2) {
        SerialBT.print(F("Record "));
        SerialBT.print(buf[i + 1], DEC);
        SerialBT.print(F("\t"));
        switch (buf[i + 2]) {
            case 0:
                SerialBT.println(F("Loaded"));
                break;
            case 0xFC:
                SerialBT.println(F("Record already in recognizer"));
                break;
            case 0xFD:
                SerialBT.println(F("Recognizer full"));
                break;
            case 0xFE:
                SerialBT.println(F("Record untrained"));
                break;
            case 0xFF:
                SerialBT.println(F("Value out of range"));
                break;
            default:
                SerialBT.println(F("Unknown status"));
                break;
        }
    }
}
...
void printTrain(uint8_t *buf, uint8_t len) {
    if (len == 0) {
        SerialBT.println(F("Train Finish."));
        return;
    } else {
        SerialBT.print(F("Train success: "));
        SerialBT.println(buf[0], DEC);
    }
    for (int i = 0; i < len - 1; i += 2) {
        SerialBT.print(F("Record "));
        ...
        switch (buf[i + 2]) {
            case 0:
                SerialBT.println(F("Trained"));
                break;
            case 0xFE:
                SerialBT.println(F("Train Time Out"));
                break;
            case 0xFF:
                SerialBT.println(F("Value out of range"));
                break;
            default:
                SerialBT.print(F("Unknown status "));
                SerialBT.println(buf[i + 2], HEX);
                break;
        }
    }
}
...
void printSigTrain(uint8_t *buf, uint8_t len) {
    if (len == 0) {
        SerialBT.println(F("Train With Signature Finish."));
        return;
    } else {
        SerialBT.print(F("Success: "));
        SerialBT.println(buf[0], DEC);
    }
    SerialBT.print(F("Record "));
    SerialBT.print(buf[1], DEC);
    ...
    switch (buf[2]) {
        case 0:
            ...
            break;
        case 0xF0:
            SerialBT.println(F("Trained, signature truncate"));
            break;
        case 0xFE:
            SerialBT.println(F("Train Time Out"));
            break;
        ...
        default:
            SerialBT.print(F("Unknown status "));
            SerialBT.println(buf[2], HEX);
            break;
    }
    SerialBT.print(F("SIG: "));
    SerialBT.write(buf + 3, len - 3);
    SerialBT.println();
}
...
void printSystemSettings(uint8_t *buf, int len) {
    ...
    switch (buf[0]) {
        case 0:
        case 3:
            SerialBT.println(F("Baud rate: 9600"));
            break;
        case 1:
            SerialBT.println(F("Baud rate: 2400"));
            break;
        case 2:
            SerialBT.println(F("Baud rate: 4800"));
            break;
        case 4:
            SerialBT.println(F("Baud rate: 19200"));
            break;
        case 5:
            SerialBT.println(F("Baud rate: 38400"));
            break;
        default:
            SerialBT.println(F("Baud rate: UNKONOWN"));
            break;
    }
    ...
    switch (buf[1]) {
        case 0:
        case 0xFF:
            SerialBT.println(F("Outpu IO Mode: Pulse"));
            break;
        case 1:
            SerialBT.println(F("Outpu IO Mode: Toggle"));
            break;
        case 2:
            SerialBT.println(F("Outpu IO Mode: Clear(When recognized) "));
            break;
        case 3:
            SerialBT.println(F("Outpu IO Mode: Set(When recognized)"));
            break;
        default:
            SerialBT.println(F("Output IO Mode: UNKONOWN"));
            break;
    }

    if (buf[2] > 15) {
        SerialBT.println(F("Pulse width: UNKONOWN"));
    } else {
        SerialBT.print(F("Pulse Width: "));
        SerialBT.print(io_pw_tab[buf[2]], DEC);
        SerialBT.println(F("ms"));
    }

    if (buf[3] == 0 || buf[3] == 0xFF) {
        SerialBT.println(F("Auto Load: disable"));
    } else {
        SerialBT.println(F("Auto Load: enable"));
    }

    switch (buf[4]) {
        case 0:
        case 0xFF:
            SerialBT.println(F("Group control by external IO: disabled"));
            break;
        case 1:
            SerialBT.println(F("Group control by external IO: system group selected"));
            break;
        case 2:
            SerialBT.println(F("Group control by external IO: user group selected"));
            break;
        default:
            SerialBT.println(F("Group control by external IO: UNKNOWN STATUS"));
            break;
    }
}
...
void printHelp() {
    printSeperator();
    SerialBT.println(F("Train records"));
    SerialBT.println(F("- train (r0) (r1)...  ex)train 0 2 45"));

    SerialBT.println(F("Load records"));
    SerialBT.println(F("- load (r0) (r1) ... ex)load 0 51 2 3"));

    SerialBT.println(F("Remove all records in  Recognizer"));
    SerialBT.println(F("- clear ex)clear"));

    SerialBT.println(F("Check record train status"));
    SerialBT.println(F("- record ex) record"));
    SerialBT.println(F("- record (r0) (r1)...  ex)record 0 79"));

    SerialBT.println(F("Check recognizer status"));
    SerialBT.println(F("- vr ex)vr"));

    SerialBT.println(F("Get signature of record (r)"));
    SerialBT.println(F("- getsig (r) ex)getsig 0"));

    SerialBT.println(F("Train one record(r) with signature(sig)"));
    SerialBT.println(F("- sigtrain (r) (sig) ex)sigtrain 0 ZERO"));

    SerialBT.println(F("Check current system settings"));
    SerialBT.println(F("- settings ex)settings"));

    SerialBT.println(F("Print this message"));
    SerialBT.println(F("- help ex)help"));
}
